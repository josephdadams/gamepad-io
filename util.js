const {app, Notification, nativeImage, systemPreferences, Menu, shell, ipcMain, BrowserWindow } = require('electron');
...
function startSocketIO() {
	...
	io.sockets.on('connection', (socket) => {		
		...
		socket.on('haptic', function(uuid, type, params) {
			...
			if (controller !== null) {
				global.win.webContents.send('haptic', controller.index, type, params);
			}
		});
	});
...
function createWindow() {
	...
	global.win.webContents.on('did-finish-load', () => {
		...
		if (global.CONTROLLERS.length < 1) {
			...
			showNotification({title: 'No gamepads found.', body: 'Please press a button on a gamepad to detect gamepads.', showNotification: true});
		}
...
function loadIPCEvents() {
	ipcMain.on('gamepad_connected', (event, index, id, totalButtons, totalAxes) => {
		showNotification({title: 'Gamepad Connected', body: id, showNotification: true});
		...
		for (let i = 0; i < storeControllers.length; i++) {
			if (storeControllers[i].id == id) {
				...
				for (let j = 0; j < global.CONTROLLERS.length; j++) {
					if (global.CONTROLLERS[j].uuid == storeControllers[i].uuid) {
						uuidInUse = true;
						break;
					}
				}
	...
	ipcMain.on('gamepad_disconnected', (event, index) => {
		...
		showNotification({title: 'Gamepad Disconnected', body: controller.id, showNotification: true});
	...
	ipcMain.on('button_event', (event, index, buttonIndex, pressed, touched, val, pct) => {
		...
		for (let i = 0; i < global.CONTROLLERS.length; i++) {
			if (global.CONTROLLERS[i].index == index) {
				for (let j = 0; j < global.CONTROLLERS[i].buttons.length; j++) {
					if (global.CONTROLLERS[i].buttons[j].buttonIndex == buttonIndex) {
						buttonExists = true;
						//now check if any of the other fields are different, so we know whether to send this button data to the clients
						if (global.CONTROLLERS[i].buttons[j].pressed == pressed && global.CONTROLLERS[i].buttons[j].touched == touched && global.CONTROLLERS[i].buttons[j].value == val && global.CONTROLLERS[i].buttons[j].percent == pct) {
							buttonChanged = false;
						}
						else {
							buttonChanged = true;
							//update in place
							global.CONTROLLERS[i].buttons[j].pressed = pressed;
							global.CONTROLLERS[i].buttons[j].touched = touched;
			...
			if (global.CONTROLLERS[i].index == index) {
				for (let j = 0; j < global.CONTROLLERS[i].axes.length; j++) {
					if (global.CONTROLLERS[i].axes[j].axisIndex == idx) {
						...
						if (global.CONTROLLERS[i].axes[j].pressed == pressedBool && global.CONTROLLERS[i].axes[j].value == axis) {
							axisChanged = false;
						}
						else {
							axisChanged = true;
...
function sendButtonEvent(uuid, buttonIndex, pressed, touched, val, pct) {
	...
	io.to(uuid).emit('button_event', uuid, buttonIndex, pressed, touched, val, pct);
...
function showNotification(notificationObj) {
	...
	if (config.get('allowNotifications')) {
		if (notificationObj.showNotification == true) {
			let NOTIFICATION_TITLE = notificationObj.title || 'gamepad-io Notification';
